...
void lcd_character(char c) {
	...
	for (unsigned char i = 0; i<char_width; i++) {
		if(i == char_width - 1) {
			...
			for (unsigned char j = 0; j<char_height; j++) {
				set_pixel(i, j, (pgm_read_byte(&(ASCII[c - 0x20][i])) & (1 << j)) >> j);
			}
		}
	}
	...
	for(int a = 0; s[a] != '\0'; a++) {
		for (unsigned char i = 0; i<char_width; i++) {
			if(i == char_width - 1) {
				...
				for (unsigned char j = 0; j<char_height; j++) {
					set_pixel(i + x, j + y, (pgm_read_byte(&(ASCII[s[a] - 0x20][i])) & (1 << j)) >> j);
				}
			}
		}
